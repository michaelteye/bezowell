  ...
  async depositWithdrawal(
    ...
    if (paymentMethod.phoneNumber) {
      ...
      if (type === TRANSACTION_TYPE.DEBIT) {
        ...
        const debitResponse = await this.transferService.userAccountWithdrawal(withdrawl, transaction);
        ...
        if (debitResponse.statusCode == TRANSFER_STATUS_CODE.SUCCESS) {
          ...
          // this.logger.info("DepositWithdrawal")
        } else {
        ...
        if (depositWithdrawalResponse.status === 'PENDING') {
          ...
          // streak goes here
          const saveStreak = await this.em.save()
  ...
  async reverseTransaction(transaction: TransactionEntity) {
    if (transaction.transactionType === TRANSACTION_TYPE.WITHDRAWAL) {
      ...
      await this.notificationService.sendSms({
        ...
        Your transaction of GHS${transaction.amount} is reversed.Thank you for choosing BezoSusu REF:${transaction.userRef}.`
      })
    }
    return transaction;
  }
